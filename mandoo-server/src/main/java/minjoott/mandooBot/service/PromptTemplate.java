...
public enum PromptTemplate {
    RAG_CONTEXT_DECISION(
            "\n# 정체성\n" +
                    ...
                    "   A) 과거 대화 내용(인칭대명사·지시어·지난 이야기 등)을 참조해서 답변해야 하는 경우\n " +
                    ...
                    "3. 단, 답변하기 위해 참고할 메시지가 챗봇 만두와의 최근 대화 기록 10개만으로 충분하다면 가능하다면, \"X\"를 출력합니다.\n" +
    ...
    RAG_CONTEXT_FILTER(
            "\n# 정체성\n" +
                    ...
                    "2. \"%s\"가 질문 또는 특정 정보를 요청하는 메시지라면, 후보에서 해당 질문·요청과 동일한 메시지만 모두 제거해주세요.\n" +
                    "3. 2번을 진행하고 남은 후보 중에서, 챗봇 만두가 \"%s\"에 답변하기 위해 참고하기에 유용한 메시지만 보내주세요. " +
                    "   예: “내 성격 맞춰줘”, “내 일정 정리해줘”와 같은 메시지를 요청받았다면, 해당 메시지에 답변하기 위해 필요한 정보들을 최대한 많이 선별해 보내주세요.\n" +
                    "4. 선별된 메시지는 날짜 및 시간이 오래된 순서로 정렬하고, 유효한 JSON 배열 형태로 출력하세요. (필드: sender, msg, dateTime 3개)\n" +
                    "   - 문자열 내부의 쌍따옴표(`\"`), 줄바꿈(`\n`) 등은 모두 `\"`, `\\n` 등으로 이스케이프 처리하세요.\n" +
